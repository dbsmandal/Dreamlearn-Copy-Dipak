...
const Courses = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                setLoading(true);
                const response = await LearnerService.registeredCourse();
                                ...
                                <div key={index} className="flex w-8/12 mt-10 m-auto p-2 border shadow-sm rounded-md shadow-purple-500 ">
                                    <div className="place-content-between w-full m-auto flex">
                                        ...
                                        <div className="my-auto">
                                            <button className="border  p-1  pl-2 pr-3 text-lg rounded-lg bg-purple-900 font-semibold text-white hover:bg-purple-700" ><Link to={`/courses/${courseTitle}`} >Schedule</Link></button>
                                        </div>
                                    </div>
